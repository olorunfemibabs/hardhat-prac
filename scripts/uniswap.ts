...
async function main() {
  ...
  const amountToReceive2 = await ethers.utils.parseEther("80");
  const amountToSwap2 = await ethers.utils.parseEther("13");
  //console.log(amountToSwap);

  console.log("token swap");

  const swapTokens = await Uniswap.connect(impersonatedSigner).swapTokensForExactTokens(
    amountToReceive2,
    amountToSwap2,
    path3,
    DAIHolder,
